...
- (void)setup
{
    ...
    CGRect pageControlFrame = CGRectMake(0.0, self.frame.size.height - 10.0, self.frame.size.width, 10.0);
...
//    NSLog(@"page....%d.",page);
    UILabel *pageLabel = [[UILabel alloc]initWithFrame:CGRectMake(self.scrollView.contentOffset.x, 0, 100, 30)];
    [pageLabel setText:[NSString stringWithFormat:@"Page %d",page]];
    [self.scrollView addSubview:pageLabel];
    self.pageControl.currentPage = page;
    
    if (self.pageControl.currentPage == (iteration - 1)) {
        NSLog(@"reload content");

        dispatch_async(dispatch_get_main_queue(), ^{
            
            CGRect tmpRect = CGRectMake(0, 0, self.frame.size.width, self.frame.size.height);        
            [self setNeedsDisplayInRect:tmpRect];
...
{
    NSLog(@"start draw text .");
    [self.pages makeObjectsPerformSelector:@selector(removeFromSuperview)];
    [self.pages removeAllObjects];

    CGRect pageControlFrame = CGRectMake(self.frame.size.width * iteration, self.frame.size.height - 20.0, self.frame.size.width, 20.0);
    self.pageControl.frame = pageControlFrame;

    BOOL moreTextAvailable = YES;
    do 
    {
        NSLog(@"ScrollView Frame %@",self.scrollView.frame);
        ...
        NSLog(@"draw");
        if (iteration > (self.pageControl.currentPage + 3)) {
            break;
        }
    } 
