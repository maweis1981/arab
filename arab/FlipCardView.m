...
- (void)setDelegate:(id)d {
    ...
    flags.delegateDidLongPressedThumbnail = [delegate respondsToSelector:@selector(flipCardView:didLongPressedThumbnailForRow:forColumn:)];
}
    ...
    if (!initialized) {
        ...
        for (NSUInteger row = 0; row < numberOfRows; row++) {
            for (NSUInteger column = 0; column < numberOfColumns; column++) {
                ...
                UILongPressGestureRecognizer *tap1 = [[UILongPressGestureRecognizer alloc] initWithTarget:self action:@selector(thumnailButtonLongPressed:)];
                //        tap1.numberOfTouchesRequired = 1; // 同时触摸数
                //        tap1.numberOfTapsRequired = 1; // 点击次数
                tap1.minimumPressDuration = 1.0; // 最小延迟（秒）
        ...
        [self bringSubviewToFront:thumbnailView];
    }
}


- (void)thumnailButtonLongPressed:(UILongPressGestureRecognizer *)sender{
    NSLog(@"button long pressed.");
    if ([sender state] == UIGestureRecognizerStateBegan) {
        FlipCardThumbnailButton *button = (FlipCardThumbnailButton *)[sender view];
        lastSelected = button;
        NSLog(@"%d %d",button.row,button.column);
        if (flags.delegateDidLongPressedThumbnail) {
            [delegate flipCardView:self didLongPressedThumbnailForRow:button.row forColumn:button.column];
        }
    }
