...
-(CTParagraphStyleRef)loadCustomParagraphStyle{
    ...
    floatValue[2] = 0.0;
    floatValue[3] = 0.0;
    floatValue[4] = 10.0; // Line spacing (必要に応じて行間調整)
...
- (id)initWithHTML:(NSData *)data options:(NSDictionary *)options documentAttributes:(NSDictionary **)dict
{
	...
	[styleSheet parseStyleBlock:@"h3 {text-align:center; font-size: 20px; font-family:STHeitiJ-Medium;}"];
	[styleSheet parseStyleBlock:@"h2 {text-align:center; font-size: 20px; font-family:STHeitiJ-Medium;}"];    
	[styleSheet parseStyleBlock:@"p {text-indent: 2em;}"];    
	// for performance we will return this mutable string
	NSMutableAttributedString *tmpString = [[NSMutableAttributedString alloc] init];
	
#if ALLOW_IPHONE_SPECIAL_CASES
	CGFloat nextParagraphAdditionalSpaceBefore = 1.0;
#endif
	...
	defaultFontDescriptor.pointSize = 14.0 * textScale;
	...
	if (defaultFontFamily)
	{
        ...
        defaultFontDescriptor.fontName = defaultFontFamily;
	}
	else
	{
		defaultFontDescriptor.fontFamily = @"STHeitiJ-Light";
            defaultFontDescriptor.fontName = @"STHeitiJ-Light";
	}
...
//	DTCoreTextParagraphStyle *defaultParagraphStyle = [DTCoreTextParagraphStyle paragraphStyleWithCTParagraphStyle:[self loadCustomParagraphStyle]];
...
//                    currentTag.paragraphStyle.headIndent = 30.0f;
                    //add blank space indent.
//                    currentTag.paragraphStyle.firstLineIndent = 30.0f;
                    
                    
                    NSLog(@"======%@",currentTag.text);
...
//				NSLog(@"Plain String is [%@]",plainString);
