...
- (id) initWithCoder: (NSCoder*) coder
{
	if ((self = [super init]) != nil) {
		chapterIndex = [[coder decodeObjectForKey: @"chapterIndex"] intValue];
		title = [coder decodeObjectForKey:@"title"];
        pageCount = [[coder decodeObjectForKey:@"pageCount"] intValue];
        spinePath = [coder decodeObjectForKey:@"spinePath"];
//        text = [coder decodeObjectForKey:@"text"];
        fontPercentSize = [[coder decodeObjectForKey:@"fontPercentSize"] intValue];

        NSLog(@"NSUserDefault load spinePath is %d %@ %d %@ %d",chapterIndex,title,pageCount,spinePath,fontPercentSize);
	}
	return self;
}
...
{
	[coder encodeObject: [NSNumber numberWithInt:chapterIndex] forKey:@"chapterIndex"];
	[coder encodeObject: title forKey:@"title"];
	[coder encodeObject: [NSNumber numberWithInt:pageCount] forKey:@"pageCount"];
	[coder encodeObject: spinePath forKey:@"spinePath"];
//	[coder encodeObject: text forKey:@"text"];
	[coder encodeObject: [NSNumber numberWithInt:fontPercentSize] forKey:@"fontPercentSize"];    
